'use strict';
const AJAX_URL = '../../app/ajax/admin/dashboard_talent.php';
let initialLoad = 1;
...
const loadTalentList = () => {
    ...
    .catch(error => {
        console.log('[ERROR] Error en loadTalentList:', error);
        iziToastError({message: 'Error de conexión. Por favor intenta de nuevo más tarde'});
    });
}

const renderCurrentData = (data) => {
    setText('talent-count', data.talent || '0');
}
...
const renderData = (items) => {
    ...
    let talentCount = 0;
    items.forEach(item => {
        talentCount = item.count || 0;
    });
    setText('talent', talentCount || '0');
}
